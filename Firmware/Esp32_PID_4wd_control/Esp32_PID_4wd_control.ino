...
function go(lin, ang, btn) {
  ...
  send();
  iv = setInterval(send, 150);  // re-send every 150ms — keeps rover alive
}
...
void handlePID() {
  ...
  // Reset integrators when gains change to avoid sudden jumps
  intFL = intFR = intRL = intRR = 0.0f;
  preFL = preFR = preRL = preRR = 0.0f;
  server.send(200, "text/plain", "OK");
}
...
void setup() {
  ...
  // STBY LOW at boot — motors cannot spin
  pinMode(STBY_FRONT, OUTPUT); digitalWrite(STBY_FRONT, LOW);
  pinMode(STBY_REAR,  OUTPUT); digitalWrite(STBY_REAR,  LOW);

  // Encoder pins — 34/35/36/39 are input-only (no pullup available)
  // 4/15/16/17 use INPUT_PULLUP for cleaner signal
  ...
  // Attach encoder interrupts on channel A of each motor
  ...
  ArduinoOTA.onStart([]() {
    stopAll();  // safety: stop motors before flash write
...
// ================================================================
void loop() {
  // 1. OTA — must be first, handles incoming firmware uploads
  ArduinoOTA.handle();

  // 2. Web server — handles button presses from browser
  server.handleClient();

  // 3. PID — runs on fixed interval, independent of web traffic
  ...
  // 4. Serial commands for calibration
  if (Serial.available()) {
    char c = Serial.read();
